...
这是《红楼梦》的 Obsidian Wiki，包含 120 回全文（繁简双版本）、106 个人物页、180 个事件页、38 个概念页、52 个红学专题综述，以及 26 张 AI 插图。
...
| 输出产品 | 13 | 速读指南、人物手册、主题导读、大观园空间手册、维护审查等 |
| raw 原始文本 | 121 | 原始文本层，单独统计，不并入成品总计 |
| **成品+raw 管理口径** | **948** | 仅用于维护统计 |
| **总计** | **827** | 成品 Wiki Markdown 文件，不含 `raw/` |

注：`827` 是成品 Wiki Markdown 口径；`raw/` 下 `121` 个原始文本 Markdown 只作为原始文本层单独统计。本轮按方案 C 暂不改变 `raw/` 的 Git 管理策略。
